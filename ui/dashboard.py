...
if data and parsed and metrics:
    ...
    with tab1:
        ...
        # WACC input - use session state keys
    ...
    with tab3:
        ...
        with col2:
            dep_pct = st.number_input("D&A % af Revenue", 0.0, 0.2, float(default_assumptions["dep_pct"][0]), 0.01, key="dep_t3")
        with col3:
            capex_pct = st.number_input("CapEx % af Revenue", 0.0, 0.3, float(default_assumptions["capex_pct"][0]), 0.01, key="capex_t3")

        nwc_pct = st.number_input("ΔNWC % af Revenue", -0.1, 0.2, float(default_assumptions["nwc_pct"][0]), 0.01, key="nwc_t3")
